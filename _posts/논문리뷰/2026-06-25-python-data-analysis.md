...
2001년 당시 주류 통계학은 '데이터가 어떤 확률모델(예: 선형회귀)로 생성되었다'고 가정하고 그 모델의 계수를 추정·검정하는 데 집중했습니다. Breiman은 이런 가정 중심 접근이 복잡한 현실 데이터에는 잘 맞지 않으며, 정작 흥미로운 실제 문제들을 외면하게 만든다고 보았습니다.
...
Breiman은 데이터 분석 문화를 두 가지로 명확히 개념화했습니다. 이 '두 문화'라는 프레임 자체가 새로운 기여입니다.
